...
//    String id, email;
    ...
    EditText et_pass1, et_pass2,et_age, et_home;
...
//            username = PreferenceManager.getString(mContext, "google_id");
//        }
//        if(email.toString() == null){
//            email = PreferenceManager.getString(mContext, "google_email");
//        }
    }
...
//                passwordChange(passwordRequest);
//                sibal(passwordRequest);
//                TestHeader(passwordRequest);
                String new_pass1 = et_pass1.getText().toString();
                String new_pass2 = et_pass2.getText().toString();
                if(new_pass1.equals(new_pass2)) {
                    PreferenceManager.setString(mContext, "new_pass1", new_pass1);
                    PreferenceManager.setString(mContext, "new_pass2", new_pass2);
                    Toast.makeText(UserActivity.this, "비밀번호가 변경되었습니다", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                }else{
                    Toast.makeText(UserActivity.this, "비밀번호가 일치하지 않습니다", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                }
            }
        });
...
//        String new_password1 = et_pass1.getText().toString();
//        String new_password2 = et_pass2.getText().toString();
//            token = PreferenceManager.getString(mContext, "key");
//            String authtoken = "Bearer " + token;
//        Call<PasswordResponse> changeResponseCall = ApiClient.getService().changepassword2(authtoken, passwordRequest);
...
//    private void sibal(PasswordRequest passwordRequest){
//        token = PreferenceManager.getString(mContext, "key");
//        String authtoken = "Bearer " + token;
...
//                Request.Builder newBuilder = request.newBuilder().header("Authorization",authtoken);
...
//        Call<PasswordResponse> call = userService.changepassword(passwordRequest);
//
...
//    public void TestHeader(PasswordRequest passwordRequest){
//        String new_password1 = et_pass1.getText().toString();
//        String new_password2 = et_pass2.getText().toString();
//
//        Retrofit retrofit = new Retrofit.Builder().baseUrl("http://192.168.0.28:8000/")
//                .addConverterFactory(GsonConverterFactory.create()).build();
//        UserService userService = retrofit.create(UserService.class);
//        token = PreferenceManager.getString(mContext, "key");
//        String m_token = "Bearer "+token;
//        Call<PasswordResponse> TestHeader = userService.changepassword5(m_token, new_password1,new_password2);
//        Log.e("m_token cheak" , "====="+m_token);
//        TestHeader.enqueue(new Callback<PasswordResponse>() {
//            @Override
//            public void onResponse(Call<PasswordResponse> call, Response<PasswordResponse> response) {
//                if( response.isSuccessful() ){
//                    String message = "비밀번호 변경에 성공하였습니다";
